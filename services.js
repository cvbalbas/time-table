...
function updateTimetableTable(cell, student, table, quantity, wss){
    var defaultRow = []
    var rowsNotUpdated
    if (quantity == "single"){
        ...
        con.query(sql, function (err, result) {
            ...
            refreshTimetableBrowserData(wss, rowsNotUpdated)
        });
    } else {
        var sql = "SELECT * FROM timetable WHERE `id` = '1'";
        con.query(sql, function (err, result) { 
            if (err) throw err;
            defaultRow = result;
...
function refreshTimetableBrowserData(wss, rowsNotUpdated){
